/*
 *  mpq-file-stream.c -- incremental logical MPQ member stream.
 *
...
static int32_t
file_stream_load_block(mpq_file_stream_s *stream, uint32_t block)
{
...
static int32_t
file_stream_adopt(
    mpq_archive_s *archive, uint32_t file_number, const char *name, mpq_file_stream_s **stream
)
{
    mpq_file_stream_s *result = NULL;
...
int32_t
libmpq__reader_file_stream_open(
...
)
{
    ...
    *stream = NULL;
    ...
    if (result != LIBMPQ_SUCCESS)
        return result;
    return file_stream_adopt(clone, file_number, NULL, stream);
}

int32_t
libmpq__reader_file_stream_open_name(
...
)
{
    ...
    *stream = NULL;
    ...
    return file_stream_adopt(clone, file_number, filename, stream);
}

int32_t
libmpq__reader_file_stream_read(
...
)
{
    ...
    while (total < size && stream->position < stream->size) {
        ...
        result = file_stream_load_block(stream, block);
        if (result != LIBMPQ_SUCCESS) {
            if (transferred != NULL)
                *transferred = total;
            return result;
...
int32_t
libmpq__reader_file_stream_tell(mpq_file_stream_s *stream, libmpq__off_t *position)
{
    if (position != NULL)
        *position = 0;
...
int32_t
libmpq__reader_file_stream_size(mpq_file_stream_s *stream, libmpq__off_t *size)
{
    if (size != NULL)
        *size = 0;
...
int32_t
libmpq__reader_file_stream_close(mpq_file_stream_s *stream)
{
