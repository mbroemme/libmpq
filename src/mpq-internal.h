...
#define LIBMPQ_FLAG_SINGLE                                                                         \
    0x01000000 /* file is stored in one single sector, first seen in world of warcraft. */
...
typedef struct
{
    uint64_t extended_offset; /* offset to the beginning of the extended block table, relative to
...
typedef struct
{
    ...
    uint32_t packed_offset_count; /* number of entries in packed_offset. */
...
typedef struct
{
    ...
    uint32_t block_table_diff;    /* block table difference between valid blocks and invalid blocks
                                     before. */
